...
describe('Platzi Punks Contract', () => {
  ...
  describe('Minting', () => {
    it('Mints a new token and assigns it to owner', async () => {
      ...
      await deployed.mint(); const ownerOfMinted = await deployed.ownerOf(0)
      expect(ownerOfMinted).to.equal(owner.address)
    })
    it('Has a minting limit', async () => {
      const maxSupply = 2; const { deployed } = await setup({ maxSupply })
      await Promise.all([deployed.mint(), deployed.mint()]); await expect(deployed.mint()).to.be.revertedWith('No PlatziPunks left :(')
    })
  })
  describe('tokenURI', () => {
